...
describe('CacheStrategyService - Adaptive TTL', () => {
  ...
  describe('Adaptive TTL Calculation', () => {
    ...
    it('should decrease TTL for high volatility keys', async () => {
      ...
      // Simulate high volatility on an analytics key (LONG base TTL)
      for (let i = 0; i < 60; i++) {
      ...
      // High volatility should reduce TTL below the LONG base tier
      expect(adaptiveTTL).toBeLessThan(CacheTTL.LONG);
      expect(adaptiveTTL).toBeGreaterThanOrEqual(
        service['adaptiveConfig'].minTTL,
      );
    });
